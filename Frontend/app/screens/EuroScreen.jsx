// app/screens/ConvertFromEuroScreen.jsx
...
const toWallets = ['GHS', 'USD', 'GBP'];

const ConvertFromEuroScreen = () => {
  ...
  const loadTransferRates = async () => {
    try {
      ...
      setTransferRates({
        EUR: { GHS: 12.13, USD: 1.10, GBP: 0.85 },
      });
    } catch (error) {
  ...
  const handleConvert = () => {
    ...
    const rate = transferRates['EUR']?.[toWallet] || 1;
  ...
  const handleTransfer = async () => {
    ...
    if (Number(amount) > balances.EUR) {
      Alert.alert('Insufficient funds', 'You do not have enough EUR to make this transfer.');
      return;
    }
    ...
    try {
      ...
      transfer('EUR', toWallet, Number(amount));
  ...
  return (
    <KeyboardAvoidingView style={[styles.container, { backgroundColor }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Text style={[styles.title, { color: textColor }]}>Transfer from Euro</Text>
...
export default ConvertFromEuroScreen;
